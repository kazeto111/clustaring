...
def k_means_pluplu(data_size, d_size, k):
  input_data = np.zeros((data_size, d_size), dtype=float)
  for i in range(data_size):
    for j in range(d_size):
      input_data[i][j] = np.random.rand()
  #print(input_data)
  ...
  for i in range(k):
      if i == 0:
          ...
          central_list[i] = np.copy(input_data[a])
      else:
          distance_matrix = np.zeros((i+1, data_size), dtype=float)
          for j in range(i+1):
              distance_matrix[j] = ((input_data - central_list[j]) ** 2).sum(axis=1)

          min_distance_list = np.min(distance_matrix, axis=0)
          #print(min_distance_list)
          p = min_distance_list/min_distance_list.sum()
          a = np.random.choice(np.arange(data_size), 1, p=p)
          if a in central_index_list:
              print("バグあり")
          else:
              central_index_list[i] = a
              central_list[i] = np.copy(input_data[a])
...
for i in data_size_list:
    for j in d_size_list:
        for l in k_list:
            print(i, j, l)
            start = time.time()
            loop_count, MSE = k_means_pluplu(i,j,l)
            end = time.time()
